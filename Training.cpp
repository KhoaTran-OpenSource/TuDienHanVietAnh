    ...
    Mat* _data, Mat* _responses)
{
    ...
    for (;;)
    {
        ...
        if (!fgets(buf, M, f) || !strchr(buf, ','))
            break;
        uint_char_union temp;
        temp.i_alpha[0] = buf[0];
        temp.i_alpha[1] = buf[1];
        temp.i_alpha[2] = buf[2];

        responses.push_back(temp.i_num);
        ptr = buf + 2;
...
static bool
build_nbayes_classifier(const string& data_filename, const string& filename_to_save, const string& filename_to_load)
{
    double t = 0;
    Mat data;
    Mat responses;
    
    bool ok = read_num_class_data(data_filename, 8, &data, &responses);
    if (!ok)
        return ok;
    ...
    //if (!filename_to_load.empty())
    //{
    //    model = load_classifier<NormalBayesClassifier>(filename_to_load);
    //    if (model.empty())
    //        return false;
    //    ntrain_samples = 0;
    //}
...
int main(int argc, char *argv[])
{
    ...
    //data_filename = "E:\\TuDienHOA\\02_UngDung\\CreateTrainingData\\chinese_letter.data";
    //if (parser.has("save"))
    ...
    //    method == 4 ?
    //    build_nbayes_classifier(data_filename, filename_to_save, filename_to_load) :
    //    method == 5 ?
    //    build_svm_classifier(data_filename, filename_to_save, filename_to_load) :
    //    -1) < 0)

    Mat image;
    image = imread(samples::findFile("E:\\TuDienHOA\\02_UngDung\\Training\\Training\\test.jpg"), IMREAD_COLOR); // Read the file
    ...
    Mat image_th;
    threshold(gray_mat, image_th, 50, 255, cv::THRESH_BINARY_INV);
